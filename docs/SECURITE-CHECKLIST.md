...
| **Mutations de contenu d'un projet gardées par la rubrique `projects`**, pas seulement leur section | Les 45 actions exportées de `actions/{tasks,taskGroups,taskCategories,taskAssignee,projectMaterials,materialCategories,projectFiles,reserves,interventions,interims,subcontractors,deliveryNoteScan}` (12 fichiers) appellent désormais `requireAreaAccess("projects")`, entre `requireCapability` et `requireSectionAccess`. ⚠️ **N'était pas le cas avant la passe adverse EDITOR (lot C1, #1)** : les lectures avaient été fermées (page projet, tableau de bord, rapport PDF, `/api/assets`) mais ces mutations non — un EDITOR dont la fonction masquait `projects` ne pouvait plus rien *voir* du chantier tout en gardant l'écriture et la suppression complètes dessus (`addTask`/`deleteTask`/`deleteMaterial`/`deleteReserve`, entre autres, suppriment réellement). `tests/authz-coverage.test.ts` vérifiait déjà `requireSectionAccess` sur ces fichiers ; un second test sur la même liste vérifie maintenant `requireAreaAccess`, pour que le trou ne se rouvre pas silencieusement. ⚠️ **Le compte a été refait à la source** (2026-09-19, feature « catégories de matériel ») plutôt que recopié : même méthode que `tests/authz-coverage.test.ts` (`functionsIn`, déclarations de fonction top-level exportées, `function` et `const … = () => {}`), appliquée fichier par fichier. `materialCategories.ts` (3 actions) a fait passer le total de 11 à 12 fichiers ; au passage, les 11 fichiers d'origine en totalisent 42 aujourd'hui et non 41 : le compte de la passe adverse était juste à `f6b422d`, `updateReserveStatusStyle` (`3255e2b`, même jour) est arrivée après sans que la ligne bouge — un compte publié sans sa date périme au premier ajout | ✅ |
