...
import type { ImportResult } from "@/actions/clients/clients";
...
export default function ImportContactsForm() {
  ...
  const [state, formAction, isPending] = useActionState<ImportResult, FormData>(
    async (_prev, formData) => importContacts(formData),
    initialState
  );
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      >
        {isPending ? t.contacts.import.importing : t.contacts.import.submit}
      </button>
          ...
          {state.errors.length > 0 && (
            <ul className="mt-2 list-disc space-y-1 pl-5">
              {state.errors.map((e, i) => (
                <li key={i}>
                  {format(t.contacts.import.rowLabel, { row: e.row })}{e.email ? ` (${e.email})` : ""} : {e.message}
                </li>
              ))}
            </ul>
          )}
