  ...
  return (
    <div className="flex items-center gap-2">
      <button
        ...
        disabled={pending}
        className="inline-flex items-center gap-1.5 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-1.5 text-sm font-medium hover:bg-gray-100 dark:hover:bg-gray-600 cursor-pointer disabled:opacity-50"
      >
        <ArrowUturnLeftIcon className="h-4 w-4" />
        {t.clients.trash.restore}
      </button>
      <button
        type="button"
      ...
      >
        ...
        {t.clients.trash.deleteForever}
      </button>

      {confirming && (
        <Modal
          title={t.clients.trash.deleteForeverTitle}
          text={format(t.clients.trash.deleteForeverText, { name })}
          error={error ?? undefined}
          textForCancel={t.common.cancel}
          textForConfirm={pending ? t.clients.deleteModal.deleting : t.clients.trash.deleteForever}
