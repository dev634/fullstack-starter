'use client'
import { addMaterialCategory } from "@/actions/materialCategories/materialCategories";
...
const initialState: MaterialCategoryActionState = {
  type: null,
  message: "",
}

// Mirror of forms/AddTaskCategoryForm.tsx for material categories (filing —
// WHAT a material is — entirely independent of the task/série/catégorie
// picker on the material form itself). Same toggle → modal → self-closing-
// on-success shape, so the two "create a category" experiences never drift
// apart for no reason.
export default function AddMaterialCategoryForm({ clientId, projectId }: { clientId: number; projectId: number }) {
  const { t } = useTranslation();
  const [open, setOpen] = useState(false);
  const [state, formAction, isPending] = useActionState<MaterialCategoryActionState, FormData>(
    addMaterialCategory,
    initialState
  );
  ...
  return (
    <>
      ...
      >
        ...
        {t.materials.category.toggle}
      </button>

      <ModalShell open={open} onClose={() => setOpen(false)} title={t.materials.category.toggle}>
